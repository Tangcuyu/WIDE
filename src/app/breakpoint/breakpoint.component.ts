...
async createWorker() {
  ...
  // 秒传的检验
  this.fileUploadService.verifyUpload(this.container.file.name, this.container.chunkstatus.hash).subscribe((d) => {
    if (!d.shouldUpload) {
      console.log('"秒传：上传成功"');
      this.status = Status.Normal;
      return;
    }
    this.shouldUpload = d.shouldUpload;
    this.uploadedList = d.uploadedList;
  });

  this.uploadData = this.container.fileChunks.map(({ file }, index): UploadData => {
    return {
      fileHash: this.container.hash[index],
      index,
      hash: this.container.hash[index] + '-' + index,
      file,
      size: file.size,
